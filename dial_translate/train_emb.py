...
logger.addHandler(logging.FileHandler('./log/train_emb'))
...
def main(args):     
    ...
    while (step < state['loop_iters'] and
        ...
        _abs = batch['Abs']
        _nat = batch['Nat']
        _noises = batch['NoiseRows']

        c = train_batch([_abs, _nat] + _noises)
        ...
        if valid_data is not None and step % state['valid_freq'] == 0 and step > 1:
            ...
            while True:
                ...
                c = eval_batch([_abs, _nat] + _noises)
...
if __name__ == "__main__":
    ...
    args = parse_args()
    args.run_id = 'embedding_emb100_h100'
