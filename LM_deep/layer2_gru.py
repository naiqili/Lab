...
class Layer2GRU(Model):
    def __init__(self, state):
        ...
        self.params = []
        self.init_params()
        self.gru1 = GRU(state, self.rng, self.emb_dim, self.h1_dim)
        self.gru2 = GRU(state, self.rng, self.h1_dim, self.h2_dim)
    ...
    def build_gen_pred(self):
        ...
        h1_t = (np.float32(1.0) - z1_t) * h1_tm1 + z1_t * h1_tilde
        ...
        h2_t = (np.float32(1.0) - z2_t) * h2_tm1 + z2_t * h2_tilde
    ...
    def build_output(self, x):
        ...
        self.hs1 = self.gru1.build_output(batch_size, xe, xmask)
        self.hs2 = self.gru2.build_output(batch_size, self.hs1, xmask)
    ...
    def genReset(self):
        self.genh1 =  theano.shared(np.zeros((self.h1_dim,), dtype='float32'), name='h_gen')
        self.genh2 =  theano.shared(np.zeros((self.h2_dim,), dtype='float32'), name='h_gen')
