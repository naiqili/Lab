...
class BiLSTMModel():
    ...
    def add_variables(self):
        ...
        with tf.variable_scope('Forward'):
            tf.get_variable('W^i_mid',
                [self.emb_size, self.fw_cell_size])
            tf.get_variable('W^i_left',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('W^i_right',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('b^i',
                [1, self.fw_cell_size])

            tf.get_variable('W^f_mid',
                [self.emb_size, self.fw_cell_size])
            tf.get_variable('W^f_left_left',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('W^f_left_right',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('W^f_right_left',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('W^f_right_right',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('b^f',
                [1, self.fw_cell_size])

            tf.get_variable('W^o_mid',
                [self.emb_size, self.fw_cell_size])
            tf.get_variable('W^o_left',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('W^o_right',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('b^o',
                [1, self.fw_cell_size])

            tf.get_variable('W^u_mid',
                [self.emb_size, self.fw_cell_size])
            tf.get_variable('W^u_left',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('W^u_right',
                [self.fw_cell_size, self.fw_cell_size])
            tf.get_variable('b^u',
                [1, self.fw_cell_size])
        with tf.variable_scope('Backward'):
            tf.get_variable('W_left',
                [self.comp_cell_size+self.de_cell_size, self.de_cell_size])
    ...
    def decompose_children(self, fb_tensor):
        ...
        left_tensor = tf.nn.relu(tf.matmul(fb_tensor, dW1) + db1)
        right_tensor = tf.nn.relu(tf.matmul(fb_tensor, dW2) + db2)
    ...
    def build_model(self, left_ts, right_ts, wv_ts, target_ts, is_leaf_ts, len_ts):
        ...
        self.build_backward(left_ts, right_ts, wv_ts, is_leaf_ts, len_ts, forward_tensors)
    ...
    def build_backward(self, left_ts, right_ts, wv_ts, is_leaf_ts, len_ts, forward_tensors):
        backward_tensors = tf.TensorArray(tf.float32, size=0, dynamic_size=True, \
            clear_after_read=False, infer_shape=False)
        input_len = tf.squeeze(len_ts)
        init_state = tf.zeros([1, self.de_cell_size])
        backward_tensors = backward_tensors.write(input_len-1, init_state)
        
        def backward_loop_body(node_tensors, forward_tensors, i):
            def fn1(node_tensors):
                parent_backward_tensor = node_tensors.read(i)
                parent_forward_tensor = forward_tensors.read(i)
                fb_tensor = tf.concat([parent_backward_tensor, parent_forward_tensor], 1)
                left_child = tf.gather(left_ts, i)
                right_child = tf.gather(right_ts, i)
                left_tensor, right_tensor = self.decompose_children(fb_tensor)
            ...
            node_tensors = tf.cond(tf.equal(tf.gather(is_leaf_ts, i), False),
                                   ...
                                   lambda: node_tensors)
            i = i - 1
            return node_tensors, forward_tensors, i

        backward_loop_cond  = lambda node_tensors, _ft, i: \
                              tf.greater_equal(i, 0)

        backward_tensors, _, _ = tf.while_loop(backward_loop_cond, backward_loop_body, [backward_tensors, forward_tensors, input_len-1], \
