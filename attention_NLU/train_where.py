...
def main(args):     
    ...
    while (step < state['loop_iters'] and
        ...
        X = batch['X']
        Xmask = batch['Xmask']
        where_in = batch['where_in']
        where_out = batch['where_out']
        wheremask = batch['wheremask']
        ...
        if True: # step % state['train_freq'] == 0:
            ...
            logger.debug(".. %.2d:%.2d:%.2d %4d mb # %d bs %d cost = %.4f acc = %.4f" % (h, m, s,\
                                                                 state['time_stop'] - (time.time() - start_time)/60., \
                                                                                         ...
                                                                                         float(c)/(wheremask==1).sum(), float(acc)))
        ...
        if valid_data is not None and step % state['valid_freq'] == 0 and step > 1:
            ...
            valid_cost = 1.0 * valid_cost/(wheremask==1).sum()
...
if __name__ == "__main__":
    ...
    args = parse_args()
    args.run_id = 'where_emb128_h128'
    args.prototype = 'where_state'
