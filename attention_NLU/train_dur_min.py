...
def main(args):     
    ...
    while (step < state['loop_iters'] and
        ...
        X = batch['X']
        Xmask = batch['Xmask']
        dur_min = batch['dur_min']

        (c, acc) = train_batch(X, Xmask, dur_min)
        #print 'Pred:', pred
        ...
        if valid_data is not None and step % state['valid_freq'] == 0 and step > 1:
            ...
            while True:
                ...
                (c, acc) = eval_batch(X, Xmask, dur_min)
...
if __name__ == "__main__":
    ...
    args = parse_args()
    args.run_id = 'dur_min_emb256_h256_f32'
    args.prototype = 'dur_min_state'
