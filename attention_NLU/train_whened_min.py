...
def main(args):     
    ...
    while (step < state['loop_iters'] and
        ...
        X = batch['X']
        Xmask = batch['Xmask']
        whened_min = batch['whened_min']

        (c, acc) = train_batch(X, Xmask, whened_min)
        #print 'Pred:', pred
        ...
        if valid_data is not None and step % state['valid_freq'] == 0 and step > 1:
            ...
            while True:
                ...
                (c, acc) = eval_batch(X, Xmask, whened_min)
...
if __name__ == "__main__":
    ...
    args = parse_args()
    args.run_id = 'whened_min_emb256_h256_f32'
    args.prototype = 'whened_min_state'
    args.resume = 'model/whenst_min_emb256_h256_f32'
    main(args)
